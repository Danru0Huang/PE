...
planner_prompt = """
   ...
   - **不要返回执行结果**，只返回计划。
    ...
    5. **value_domain**：根据property{property}所属的value_domain{value_domain}生成，如申请日期的值域为日期。如果property包含可枚举的值 {values}，请单独分开注册这些值。
    ...
    **案例1**
    ...
    2. 注册property：使用 register_property 工具，格式为：
    register_property(property_name="法律状态")
    3. 注册concept_domain：使用 register_concept_domain 工具，格式为：
    register_concept_domain(concept_domain="法律状态")
    ...
    register_data_element_concept_with_relationships(
        ...
        property="法律状态",
        concept_domain="法律状态")
    5. 注册value_domain，与1个类有关，是concept_domain关联，存在可枚举的值"1，A，有效；0，B，无效"也需要进行注册：使用 register_value_domain_with_relationship 工具，格式为：
    register_value_domain_with_values(
        value_domain="法律状态", 
        concept_domain="法律状态",
